...
One migration is built in: entries written by emboss < 0.12 were keyed on docstring-*sensitive* source, so the decorator always accepts that identity as an implicit fallback (tried after any explicit `also_accept` tokens) and copies hits forward — upgrading, or editing a docstring, keeps the cache warm with no action needed. The one edge this doesn't cover is a **rename combined with** the < 0.12 boundary: `emboss id --rev <old-rev>` computes the *current* (docstring-stripped) scheme, so to migrate a renamed function's pre-0.12 entries, derive the old token by hand (`md5` of the AST round-trip *without* docstring stripping).
